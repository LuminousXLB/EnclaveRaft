...
public:
    er_key_store(const sgx_ec256_private_t &private_key, const sgx_ec256_public_t &public_key) :
            self_private(private_key), self_public(public_key) {}
    ...
    bool register_report(const char *http_packet) {
        ...
        if (memcmp(report_data.d, &self_public, sizeof(sgx_ec256_public_t)) == 0) {
            self_report = http_packet;
            return true;
        } else {
            return false;
        }
...
private:
    ...
    string self_report;
    sgx_ec256_private_t self_private;
    sgx_ec256_public_t self_public;
};
