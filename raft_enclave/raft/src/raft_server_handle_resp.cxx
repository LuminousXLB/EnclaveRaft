...
void raft_server::handle_peer_resp(ptr<resp_msg> &resp, const ptr<rpc_exception> &err) {
    ...
    l_->debug(lstrfmt("Response from peer %d -> [%s] with Result=%d, Term=%llu, NextIndex=%llu")
                      .fmt(resp->get_src(),
    ...
    switch (resp->get_type()) {
        ...
        case msg_type::install_snapshot_response:
            ...
            break;
        default:
            string line = sstrfmt("Received an unexpected message %s for response, system exits.")
...
void raft_server::handle_append_entries_resp(resp_msg &resp) {
    ...
    if (resp.get_accepted()) {
        {
            std::lock_guard<std::mutex> lock(p->get_lock());
...
void raft_server::handle_ext_resp(ptr<resp_msg> &resp, const ptr<rpc_exception> &err) {
    ...
    l_->debug(lstrfmt("Response from peer %d: [%s] extended with Result=%d, Term=%llu, NextIndex=%llu")
                      .fmt(resp->get_src(),
                           msg_type_string(resp->get_type()),
                           resp->get_accepted() ? 1 : 0,
                           resp->get_term(),
    ...
    switch (resp->get_type()) {
        ...
        case msg_type::install_snapshot_response: {
            ...
            break;
        default:
            string line = lstrfmt("received an unexpected response message type %s, for safety, stepping down")
                    .fmt(msg_type_string(resp->get_type()));
            l_->err(line);
            ctx_->state_mgr_->system_exit(-1);
            throw raft_exception(line);
